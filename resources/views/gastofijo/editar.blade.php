...
<div class="max-w-lg mx-auto bg-white rounded-xl shadow p-6">
    <h2 class="text-xl font-bold text-gray-700 mb-6">Editar Gasto Fijo</h2>
    ...
    <form action="/gasto-fijo/{{ $gastoFijo->id }}" method="POST">
        @csrf
        @method('PUT')

        <label class="text-sm text-gray-600">Descripción:</label>
        <input type="text" name="descripcion" value="{{ old('descripcion', $gastoFijo->descripcion) }}"
            class="w-full border border-gray-300 rounded px-3 py-2 mt-1 mb-4">

        <label class="text-sm text-gray-600">Monto:</label>
        <input type="number" name="monto" step="0.01" value="{{ old('monto', $gastoFijo->monto) }}"
